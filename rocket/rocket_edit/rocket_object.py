...
# 定数設定
MAX_SAFE_SPEED = 0.5  # 安全にぶら下がれる最大速度（デフォルト値）
...
class RocketObject:
    ...
    def __init__(self, x, y, width, height, ground_y):
        ...
        self.pendulum_damping = 0.98  # 振り子の減衰係数（大きいほど長くぶらぶらする）
        self.pivot_x = 0.0  # ぶら下がり時の支点X座標
        self.pivot_y = 0.0  # ぶら下がり時の支点Y座標
    ...
    def update_position(self, gravity):
        ...
        if self.hanging:
            ...
            # 角度が変わったので、ロケットの中心位置を再計算
            # ロケットの中心からぶら下がり判定点までの距離
            offset = self.height * 0.1  # 上端から1/10の位置
            distance = self.height / 2 - offset
            ...
            # 支点からロケットの中心への相対位置
            rel_x = -distance * cos_a
            rel_y = -distance * sin_a
            
            # ロケットの中心位置を計算
            self.x = self.pivot_x + rel_x
            self.y = self.pivot_y + rel_y
    ...
    def check_chopstick_collision(self, chopstick_x, chopstick_y, chopstick_width, chopstick_length):
        ...
        if in_x_range and in_y_range:
            ...
            # ロケットの角度を調整（垂直上向きから少し傾ける）
            self.angle = -math.pi / 2 + math.radians(5)  # 上向きから5度傾ける
            
            # 初期の振り子の角速度を設定（ぶらぶらさせるため）
            self.pendulum_angle_velocity = 0.03
            
            # 支点座標を設定（チョップスティックの中央）
            self.pivot_x = self.hanging_point_x
            self.pivot_y = chopstick_y + chopstick_width / 2
